...
нарушить его работу. Меню анонимного пользователя храниться в SQL
...
- простые и уникальные индексы (unique index, index);
- проверки (checks);
- префиксы таблиц.
...
Был принят отказ от поля типа timestamp.
...
При добавлении данных на сервер, даты следует преобразовывать
к часовому поясу UTC±0:00).
